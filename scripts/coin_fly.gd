...
func setup(p_start: Vector2, p_value: int) -> void:
	...
	control_point = mid + Vector2(randf_range(-60, 60), -80 - randf_range(0, 60))
...
func _draw() -> void:
	...
	# Gold coin circle
	draw_circle(Vector2.ZERO, 6.0, Color(0.9, 0.7, 0.1, alpha))
	draw_circle(Vector2.ZERO, 4.0, Color(1.0, 0.85, 0.3, alpha))
	# $ symbol
	draw_string(ThemeDB.fallback_font, Vector2(-4, 4), "$", HORIZONTAL_ALIGNMENT_CENTER, -1, 10, Color(0.6, 0.4, 0.0, alpha))
